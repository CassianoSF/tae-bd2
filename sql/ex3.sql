...
SELECT TRIM(titulo), ano FROM Filmes
...
SELECT TRIM(titulo) FROM Filmes
...
SELECT COUNT(filme_id) as qtde_filmes, trim(lingua) FROM Linguas
...
SELECT 
	COUNT(pessoa_id) AS qtd_de_filmes, 
	Pessoas.id, 
	trim(Pessoas.nome), 
	trim(Pessoas.sexo),
	Pessoas.data_nasc 
...
SELECT trim(Pessoas.nome), trim(Tipos.descricao), trim(Creditos.personagem) FROM Pessoas
JOIN Creditos     ON Creditos.pessoa_id    = Pessoas.id
JOIN Tipos        ON Creditos.tipo_id      = Tipos.id
JOIN Filmes       ON Creditos.filme_id     = Filmes.id
